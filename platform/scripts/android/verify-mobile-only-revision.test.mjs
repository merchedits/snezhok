...
import { isMobileOnlyPath, lockfileDiffIsMobileVersionOnly, packageManifestDiffIsVerifierOnly, parseArguments } from "./verify-mobile-only-revision.mjs";
...
test("package lock fast path permits only the mobile version field", () => {
  const before = JSON.stringify({ packages: { "apps/mobile": { version: "1.0.0", dependencies: { react: "1" } } } });
  const versionOnly = JSON.stringify({ packages: { "apps/mobile": { version: "1.0.1", dependencies: { react: "1" } } } });
  const dependencyChange = JSON.stringify({ packages: { "apps/mobile": { version: "1.0.1", dependencies: { react: "2" } } } });
  assert.equal(lockfileDiffIsMobileVersionOnly(before, versionOnly), true);
  assert.equal(lockfileDiffIsMobileVersionOnly(before, dependencyChange), false);
});
