...
POSTGRES_VERIFY_IMAGE=${POSTGRES_VERIFY_IMAGE:-postgres:17-alpine}
backup_dir=${1:-}
...
for command in age docker flock sha256sum tar zstd sort comm mktemp; do require_command "$command"; done
...
age --decrypt --identity "$AGE_IDENTITY_FILE" "$backup_dir/media.tar.zst.age" \
  | tar --zstd -tf - \
  | sed 's#^\./##' \
  | sort -u >"$temporary_dir/media-files"
...
docker exec "$container" psql --username=postgres --dbname=snezhok_verify --tuples-only --no-align \
  --command="SELECT storage_key FROM blobs ORDER BY storage_key;" >"$temporary_dir/referenced-media"
