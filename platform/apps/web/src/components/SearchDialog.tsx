...
export function SearchDialog() {
  ...
  const run = async (event?: FormEvent) => {
    ...
    setError(null);
    try {
      setResults(await api.search(query.trim(), kind ? { kind } : {}));
    } catch (cause) {
  ...
  useEffect(() => {
    ...
    // Search is deliberately debounced from query and kind.
  ...
  return (
    <Dialog title="Search" onClose={() => app.setSearchOpen(false)} className="search-dialog">
      <form className="search-form" onSubmit={run}>
        ...
        <select value={kind} onChange={(event) => setKind(event.target.value)} aria-label="Media type"><option value="">All types</option><option value="media">Media</option><option value="file">Files</option><option value="voice">Voice notes</option><option value="video-note">Video notes</option></select>
      </form>
      <div className="search-results">
        ...
        {results?.people.length ? <section><h3>People</h3>{results.people.map((person) => <button className="search-result" key={person.id}><Avatar user={person} size={36} /><span><strong>{person.displayName}</strong><small>@{person.username}</small></span></button>)}</section> : null}
