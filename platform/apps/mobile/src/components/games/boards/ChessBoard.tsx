...
// Layered silhouettes produce a crisp, bold Staunton-like outline without
// copying another product's proprietary artwork or relying on text shadows.
      ...
      {squares.map((square, index) => {
        ...
        return (
          <Pressable accessibilityRole="button" accessibilityLabel={`${square}${piece ? ` ${piece.color}${piece.type}` : ""}`} key={square} onPress={() => void press(square)} style={[styles.square, { backgroundColor: selected === square ? (isLight ? "#F5F682" : "#B9CA43") : isLight ? "#EEEED2" : "#769656" }]}>
            ...
            {piece ? <ChessPieceGlyph glyph={GLYPHS[`${piece.color}${piece.type}` as keyof typeof GLYPHS]} color={piece.color} size={size / 9.35} /> : null}
          </Pressable>
        );
      })}
      <Text pointerEvents="none" style={styles.cornerLabel}>{state.inCheck ? (language === "ru" ? "ШАХ" : "CHECK") : ""}</Text>
    </View>
    {promotion ? <View style={styles.promotion}><Text style={styles.promotionLabel}>{language === "ru" ? "Превратить пешку в" : "Promote pawn to"}</Text>{(["q", "r", "b", "n"] as const).map((piece) => <Pressable accessibilityRole="button" key={piece} onPress={() => void promote(piece)} style={styles.promotionChoice}><ChessPieceGlyph glyph={GLYPHS[`${ownColor}${piece}` as keyof typeof GLYPHS]} color={ownColor} size={31} /></Pressable>)}</View> : null}
    </View>
  );
});

function ChessPieceGlyph({ glyph, color, size }: { glyph: string; color: "w" | "b"; size: number }) {
  const lineHeight = size * 1.08;
  return (
    <View pointerEvents="none" style={[styles.pieceFrame, { width: size * 1.08, height: lineHeight }]}>
      <Text allowFontScaling={false} style={[styles.pieceLayer, { color: color === "w" ? "#34312C" : "#141413", fontSize: size, lineHeight }]}>{glyph}</Text>
      <Text allowFontScaling={false} style={[styles.pieceLayer, { color: color === "w" ? "#FAF8F0" : "#373530", fontSize: size * 0.91, lineHeight }]}>{glyph}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  pieceFrame: { zIndex: 2, alignItems: "center", justifyContent: "center" },
  pieceLayer: { position: "absolute", width: "100%", textAlign: "center", fontWeight: "900" },
