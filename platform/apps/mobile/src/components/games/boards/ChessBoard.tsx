  ...
  return (
    ...
    <View style={[styles.board, { width: size, height: size }]}>
      {squares.map((square, index) => {
        ...
        return (
          <Pressable accessibilityRole="button" accessibilityLabel={`${square}${piece ? ` ${piece.color}${piece.type}` : ""}`} key={square} onPress={() => void press(square)} style={[styles.square, { backgroundColor: selected === square ? "#D7FF29" : isLight ? "#EEE5D3" : "#8467B8" }]}>
            {target ? <View style={[styles.target, piece && styles.capture]} /> : null}
            {piece ? <Text style={[styles.piece, { fontSize: size / 11 }, piece.color === "w" ? styles.whitePiece : styles.blackPiece]}>{GLYPHS[`${piece.color}${piece.type}` as keyof typeof GLYPHS]}</Text> : null}
          </Pressable>
        );
      })}
...
const styles = StyleSheet.create({
  ...
  piece: { zIndex: 2, lineHeight: 39 }, whitePiece: { color: "#FFFCF4", textShadowColor: "#2E2537", textShadowRadius: 1, textShadowOffset: { width: 0, height: 1 } }, blackPiece: { color: "#201927" },
  target: { position: "absolute", width: 13, height: 13, borderRadius: 8, backgroundColor: "rgba(23,19,26,0.28)" }, capture: { width: "84%", height: "84%", borderWidth: 4, borderColor: "rgba(215,255,41,0.7)", backgroundColor: "transparent" },
