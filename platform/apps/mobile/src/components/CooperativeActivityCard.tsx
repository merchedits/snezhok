...
const meta: Record<CooperativeActivityType, { fill: string; ink: string; icon: AppIconName; ru: string; en: string }> = {
  question: { fill: "#CDB5FF", ink: "#5D3B93", icon: "help-circle-outline", ru: "Вопрос для двоих", en: "Question Drop" },
  blitz: { fill: "#FF9184", ink: "#782C28", icon: "bolt-outline", ru: "Блиц", en: "60-Second Blitz" },
  "tiny-quest": { fill: "#DCEF72", ink: "#405000", icon: "camera", ru: "Маленький квест", en: "Tiny Quest" },
  "color-hunt": { fill: "#91E3BB", ink: "#155C3B", icon: "color-palette-outline", ru: "Охота за цветом", en: "Color Hunt" },
  "song-exchange": { fill: "#A8D8FF", ink: "#174C75", icon: "music-outline", ru: "Обмен песнями", en: "Song Exchange" },
  "movie-list": { fill: "#FFE88A", ink: "#6A5300", icon: "movie-outline", ru: "Наши фильмы", en: "Movie List" },
  "draw-guess": { fill: "#FFA044", ink: "#6B3100", icon: "pencil-outline", ru: "Нарисуй и угадай", en: "Draw & Guess" },
  "ideas-jar": { fill: "#FFB8C3", ink: "#782C48", icon: "bulb-outline", ru: "Банка идей", en: "Ideas Jar" },
  "memory-capsule": { fill: "#FFE88A", ink: "#6A5300", icon: "archive-outline", ru: "Капсула памяти", en: "Memory Capsule" },
  milestone: { fill: "#91E3BB", ink: "#155C3B", icon: "sparkles-outline", ru: "Общее достижение", en: "Shared milestone" },
};
...
export const CooperativeActivityCard = memo(function CooperativeActivityCard({ activity, onOpen }: { activity: CooperativeActivity; onOpen: () => void }) {
  ...
  const actionable = !["declined", "expired", "cancelled"].includes(activity.state);
  ...
  return (
    <View style={[styles.card, { backgroundColor: style.fill }]}>
      <View style={styles.topline}>
        <View style={[styles.icon, { backgroundColor: "rgba(255,255,255,0.48)" }]}><AppIcon name={style.icon} size={20} color={style.ink} /></View>
        <Text style={[styles.label, { color: style.ink }]}>{language === "ru" ? style.ru : style.en}</Text>
        {activity.config.secret === true && activity.state !== "completed" ? <View style={styles.secret}><AppIcon name="lock-closed-outline" size={13} color={style.ink} /><Text style={[styles.secretText, { color: style.ink }]}>{language === "ru" ? "тайно" : "secret"}</Text></View> : null}
      </View>
...
function ActivityPreview({ activity, ink, language }: { activity: CooperativeActivity; ink: string; language: "ru" | "en" }) {
  ...
  if (activity.type === "movie-list" || activity.type === "ideas-jar") {
    ...
    return <Text style={[styles.count, { color: ink }]}>{count} · {label.toLocaleLowerCase()}</Text>;
  }
  if (activity.type === "song-exchange" && activity.state === "completed") return <View style={styles.pairs}>{activity.entries.slice(0, 2).map((entry) => <Pressable key={entry.id} onPress={() => typeof entry.payload.url === "string" && void Linking.openURL(entry.payload.url)} style={styles.pair}><AppIcon name="play" size={15} color={ink} /><Text numberOfLines={1} style={[styles.pairText, { color: ink }]}>{String(entry.payload.title ?? "Song")}</Text></Pressable>)}</View>;
  if (activity.type === "tiny-quest" && activity.state === "completed") return <View style={styles.photoRow}>{activity.entries.flatMap((entry) => entry.attachments).slice(0, 2).map((attachment) => <Photo key={attachment.id} url={attachment.thumbnailUrl ?? attachment.url} wide />)}</View>;
  if (activity.state === "completed" && activity.type === "question") return <View style={styles.answers}>{activity.entries.map((entry) => <Text key={entry.id} style={[styles.answer, { color: ink }]}>{String(entry.payload.answer ?? "")}</Text>)}</View>;
  if (activity.state === "locked") return <View style={styles.locked}><AppIcon name="lock-closed-outline" size={24} color={ink} /><Text style={[styles.previewText, { color: ink }]}>{language === "ru" ? "Откроется в назначенный день" : "Opens on its reveal day"}</Text></View>;
  return null;
}
...
const styles = StyleSheet.create({
  ...
  topline: { minHeight: 34, flexDirection: "row", alignItems: "center", gap: 8 }, icon: { width: 34, height: 34, borderRadius: 12, alignItems: "center", justifyContent: "center" },
  label: { flex: 1, fontSize: 13, fontWeight: "800" }, secret: { flexDirection: "row", alignItems: "center", gap: 3 }, secretText: { fontSize: 11, fontWeight: "700" },
  prompt: { fontSize: 20, lineHeight: 25, fontWeight: "800", marginTop: 14, marginBottom: 12 }, count: { fontSize: 28, fontWeight: "800", marginVertical: 12 },
  people: { flexDirection: "row", alignItems: "center", minHeight: 35, marginTop: 10 }, person: { marginRight: -5 }, dot: { position: "absolute", right: -1, bottom: -1, width: 10, height: 10, borderRadius: 5, borderWidth: 2 }, state: { flex: 1, marginLeft: 14, fontSize: 12, fontWeight: "700", textAlign: "right" },
  action: { marginTop: 12, height: 46, borderRadius: 15, paddingHorizontal: 15, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, actionText: { color: "#FFFFFF", fontSize: 14, fontWeight: "800" },
  colorLine: { flexDirection: "row", alignItems: "center", gap: 7, marginTop: 10 }, colorDot: { width: 17, height: 17, borderRadius: 9 }, previewText: { fontSize: 13, lineHeight: 18, fontWeight: "700" },
  photoGrid: { flexDirection: "row", flexWrap: "wrap", gap: 3, marginTop: 10 }, photo: { width: 82, height: 82, borderRadius: 9 }, photoRow: { flexDirection: "row", gap: 5, marginTop: 8 }, widePhoto: { width: 133, height: 100, borderRadius: 12 },
