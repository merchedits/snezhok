...
export function ForwardPickerModal({ visible, conversations, busy, onClose, onSelect }: {
  visible: boolean;
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" statusBarTranslucent onRequestClose={onClose}>
      <View style={styles.layer}>
        ...
        <View style={[styles.sheet, { paddingBottom: Math.max(insets.bottom, 12), backgroundColor: palette.elevated, borderColor: palette.border }]}>
          <View style={[styles.handle, { backgroundColor: palette.faintText }]} />
          <View style={styles.header}>
            <Pressable onPress={onClose} style={styles.headerButton}><Ionicons name="close" size={24} color={palette.accent} /></Pressable>
            <Text style={[styles.title, { color: palette.text }]}>{t("forward")}</Text>
            <View style={styles.headerButton} />
          </View>
          <FlatList
            data={conversations.filter((conversation) => !conversation.archived)}
            ...
            renderItem={({ item }) => {
              ...
              return (
                <Pressable disabled={busy} onPress={() => onSelect(item)} style={({ pressed }) => [styles.target, { backgroundColor: pressed ? palette.surface : "transparent", opacity: busy ? 0.55 : 1 }]}>
                  {item.saved
                    ? <View style={[styles.savedAvatar, { backgroundColor: palette.accent }]}><Ionicons name="bookmark" size={20} color="white" /></View>
