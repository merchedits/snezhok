...
export function MentionsModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const open = (message: Message) => {
    ...
    onClose();
    requestAnimationFrame(() => navigation.navigate("Chat", { streamId: message.streamId, streamKind: message.streamKind, title: conversation?.title ?? pc("mentions"), targetMessageId: message.id, openedAt: performance.now() }));
  };
