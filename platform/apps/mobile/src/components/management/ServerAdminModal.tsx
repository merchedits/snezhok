...
import { productApi, type ServerBan, type ServerDetails, type ServerMemberView } from "../../lib/productApi";
...
export function ServerAdminModal({ visible, serverId, initialTab = "overview", onClose }: { visible: boolean; serverId: string | null; initialTab?: Tab; onClose: () => void }) {
  ...
  const memberMenu = (member: ServerMemberView) => {
    ...
    if (permissions.has("manage_members")) actions.push(...assignableRoles.filter((role) => role !== member.role).map((role) => ({ text: memberRoleLabel(role, pc), onPress: () => void run(() => productApi.updateServerMember(serverId, member.user.id, { role })) })));
    if (owner) actions.push({ text: pc("transferOwnership"), style: "destructive", onPress: () => void run(() => productApi.transferServer(serverId, member.user.id)) });
    if (permissions.has("kick_members")) actions.push({ text: pc("kick"), style: "destructive", onPress: () => void run(() => productApi.kickServerMember(serverId, member.user.id)) });
    if (permissions.has("ban_members")) actions.push({ text: pc("ban"), style: "destructive", onPress: () => void run(() => productApi.banServerMember(serverId, member.user.id)) });
  ...
  const submitEditor = async (value: string) => {
    if (!serverId) return;
    if (editor?.type === "server") await productApi.updateServer(serverId, { name: value });
    if (editor?.type === "category") editor.id ? await productApi.updateCategory(serverId, editor.id, { name: value }) : await productApi.createCategory(serverId, value);
    if (editor?.type === "channel") editor.id ? await productApi.updateChannel(serverId, editor.id, { name: value }) : await productApi.createChannel(serverId, { name: value, kind: editor.kind ?? "text", categoryId: null, topic: "" });
    if (editor?.type === "memberSearch") { const users = await api.searchUsers(value); const username = value.replace(/^@/, "").toLowerCase(); const user = users.find((item) => item.username.toLowerCase() === username); if (!user) throw new Error(pc("userNotFound")); await productApi.addServerMember(serverId, user.id); }
    if (editor?.type === "role") { const role = await productApi.createServerRole(serverId, { name: value, color: null, permissions: [] }); setPermissionRole(role); }
  ...
  const transferActions: AppDialogAction[] = [
    ...members.filter((item) => item.user.id !== me?.id).map((item) => ({ text: item.user.displayName, onPress: () => { if (serverId) void run(() => productApi.transferServer(serverId, item.user.id)); } })),
  ...
  return <ManagementModal visible={visible} title={server?.name ?? pc("serverManagement")} onClose={onClose} busy={busy}>
    ...
    {tab === "overview" ? <ManagementScroll><ManagementSection>
      ...
      {!owner && me && serverId ? <ManagementRow icon="log-out-outline" label={pc("leaveServer")} destructive onPress={() => showDialog(pc("leaveServer"), undefined, [{ text: pc("cancel"), style: "cancel" }, { text: pc("leaveServer"), style: "destructive", onPress: () => void run(async () => { await productApi.leaveServer(serverId, me.id); onClose(); }) }])} /> : null}
      {owner && serverId ? <ManagementRow icon="trash-outline" label={pc("deleteServer")} destructive onPress={() => showDialog(pc("deleteServer"), undefined, [{ text: pc("cancel"), style: "cancel" }, { text: pc("deleteServer"), style: "destructive", onPress: () => void run(async () => { await productApi.deleteServer(serverId); await refresh({ force: true }); onClose(); }, false) }], { dismissible: false })} /> : null}
    </ManagementSection></ManagementScroll> : null}
    {tab === "channels" ? <ManagementScroll><ManagementSection title={pc("categories")}>{categories.map((category) => <ManagementRow key={category.id} icon="folder-outline" label={category.name} {...(permissions.has("manage_categories") ? { onPress: () => showDialog(category.name, undefined, [{ text: pc("edit"), onPress: () => setEditor({ type: "category", id: category.id }) }, { text: pc("delete"), style: "destructive", onPress: () => { if (serverId) void run(() => productApi.deleteCategory(serverId, category.id)); } }, { text: pc("cancel"), style: "cancel" }]) } : {})} />)}{permissions.has("manage_categories") ? <ManagementRow icon="add" label={pc("addCategory")} onPress={() => setEditor({ type: "category" })} /> : null}</ManagementSection><ManagementSection title={pc("channels")}>{channels.map((channel) => <ManagementRow key={channel.id} icon={channel.kind === "voice" ? "volume-medium-outline" : "chatbubble-outline"} label={channel.name} detail={channel.topic} {...(permissions.has("manage_channels") ? { onPress: () => showDialog(channel.name, undefined, [{ text: pc("edit"), onPress: () => setEditor({ type: "channel", id: channel.id }) }, ...(permissions.has("manage_roles") ? [{ text: pc("overrides"), onPress: () => setOverrideChannelId(channel.id) }] : []), { text: pc("delete"), style: "destructive", onPress: () => { if (serverId) void run(() => productApi.deleteChannel(serverId, channel.id)); } }, { text: pc("cancel"), style: "cancel" }]) } : {})} />)}{permissions.has("manage_channels") ? <ManagementRow icon="add" label={pc("addChannel")} onPress={() => showDialog(pc("addChannel"), undefined, [{ text: pc("textChannel"), onPress: () => setEditor({ type: "channel", kind: "text" }) }, { text: pc("voiceChannel"), onPress: () => setEditor({ type: "channel", kind: "voice" }) }, { text: pc("cancel"), style: "cancel" }])} /> : null}</ManagementSection></ManagementScroll> : null}
    ...
    {tab === "bans" ? <FlatList contentContainerStyle={styles.list} data={bans} keyExtractor={(item) => item.user.id} renderItem={({ item }) => <ManagementRow icon="ban-outline" label={item.user.displayName} detail={item.reason} value={pc("unban")} onPress={() => serverId && void run(async () => { await productApi.unbanServerMember(serverId, item.user.id); setBans(await productApi.serverBans(serverId)); }, false)} />} ListEmptyComponent={<ManagementEmpty text={pc("noItems")} />} /> : null}
...
function RolePermissionModal({ visible, serverId, role, canDelete, onClose, onSaved }: { visible: boolean; serverId: string | null; role: ServerRoleDefinition | null; canDelete: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => { if (!serverId || !role) return; setBusy(true); try { await productApi.updateServerRole(serverId, role.id, { permissions }); onSaved(); onClose(); } catch (error) { showDialog(pc("operationFailed"), userFacingError(error, t)); } finally { setBusy(false); } };
  const remove = async () => { if (!serverId || !role) return; setBusy(true); try { await productApi.deleteServerRole(serverId, role.id); onSaved(); onClose(); } catch (error) { showDialog(pc("operationFailed"), userFacingError(error, t)); } finally { setBusy(false); } };
...
function ChannelOverridesModal({ visible, serverId, channelId, roles, members, onClose }: { visible: boolean; serverId: string | null; channelId: string | null; roles: ServerRoleDefinition[]; members: ServerMemberView[]; onClose: () => void }) {
  ...
  const load = useCallback(async () => { if (serverId && channelId) setItems(await productApi.channelOverrides(serverId, channelId)); }, [channelId, serverId]); useEffect(() => { if (visible) void load().catch((error) => showDialog(pc("operationFailed"), userFacingError(error, t))); else setTarget(null); }, [load, visible]);
  ...
  const save = async () => { if (!serverId || !channelId || !target) return; setBusy(true); try { if (allow.length || deny.length) await productApi.setChannelOverride(serverId, channelId, target.type, target.id, { allow, deny }); else await productApi.removeChannelOverride(serverId, channelId, target.type, target.id); await load(); setTarget(null); } catch (error) { showDialog(pc("operationFailed"), userFacingError(error, t)); } finally { setBusy(false); } };
