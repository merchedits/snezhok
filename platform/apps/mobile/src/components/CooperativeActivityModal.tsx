...
import { ActivityIndicator, Animated, Linking, Modal, PanResponder, Pressable, ScrollView, StyleSheet, Text, TextInput, View } from "react-native";
...
export function CooperativeActivityModal({ message, onClose }: { message: Message | null; onClose: () => void }) {
  ...
  const uploadSelection = async (inputs: UploadInput[]) => {
    ...
    try {
      ...
      for (const input of inputs.slice(0, limit)) attachments.push(await upload(input));
...
function CollagePhoto({ attachment }: { attachment: CooperativeActivityEntry["attachments"][number] }) {
  const [open, setOpen] = useState(false);
...
function ReadOnlyDrawing({ strokes }: { strokes: number[][][] }) {
  return (
...
function Primary({ label, disabled, busy, onPress }: { label: string; disabled: boolean; busy: boolean; onPress: () => void }) {
  const palette = usePalette();
  return (
    <Pressable
      accessibilityRole="button"
