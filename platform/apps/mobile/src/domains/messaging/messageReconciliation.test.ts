...
test("message reconciliation drops irrecoverable entries and repairs a legacy media URL", () => {
  ...
  assert.equal(normalized?.attachments[0]?.url, "/api/v1/files/missing-url");
});
