...
The Android client keeps a local database for recent conversations, the latest five normal message pages per stream, read state, drafts, and transfer queues. The web client uses durable browser storage for the cached shell, recent messages, drafts, and pending sends.
