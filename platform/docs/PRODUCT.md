...
1. Familiar before novel. A screen or interaction should map directly to a proven Telegram or Discord pattern.
...
- **Servers**: member communities containing categorized channels.
- **Text channel**: persistent server message stream.
- **Voice channel**: persistent joinable server call room.
...
The authenticated shell has three permanent regions and one optional region:

1. A 64 px server rail.
2. A 280 px contextual sidebar.
3. A flexible content pane.
4. An optional 320 px information or member drawer, closed by default.

The server rail contains Home, ordered server icons, and Add server. Home changes the contextual sidebar to Chats. Selecting a server changes it to categories and channels. Server names appear in tooltips; labels do not crowd the rail.

The contextual sidebar contains only navigation related to the current rail selection. The information drawer never consumes permanent space unless the user opens it, and its open state is remembered per conversation or channel.

The content header is 52 px high. It contains the current avatar or channel icon, title, concise status or topic when present, then Search, Call or Join, and Info actions. It does not contain marketing copy.
...
3. Per-channel or per-chat overrides.

Each layer supports all messages, mentions only where relevant, mute duration, preview visibility, sound, and mobile or desktop delivery. Quiet hours have explicit start, end, and day selection.
...
Server voice channels are persistent rooms. Users can continue browsing messages while connected. A compact call bar remains at the bottom of the sidebar or navigation drawer and shows channel, connection quality, microphone, and disconnect controls.
