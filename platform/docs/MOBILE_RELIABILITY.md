...
- Android chat uses `adjustResize` as the native viewport owner. The header stays outside a height-only keyboard-avoiding timeline/composer region, which is a no-op when native resize already supplied the inset and a guard when an OEM edge-to-edge window does not. Never translate that region. FlashList's bottom-autoscroll threshold is a viewport ratio (`0.2`), not pixels; oversized values force history readers to the newest message. Composer safe-area padding still interpolates from the keyboard controller's UI-thread progress and must not depend on post-animation `keyboardDidShow`/`keyboardDidHide` callbacks.
