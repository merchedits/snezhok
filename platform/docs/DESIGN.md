...
Messages dominate. Incoming bubbles are Warm with Ink; outgoing bubbles are Violet with white. Bubbles have no normal border or shadow and use a tighter sender-side top corner. Single photo/video messages are content-first cards: no colored bubble padding, only a hairline neutral edge, the complete source aspect ratio within safe viewport bounds, and one translucent bottom-right island for time and delivery state. Media reactions occupy a matching bottom-left island. The entire attachment surface opens the viewer; only the empty row gutter outside it invokes reactions. Text-message reactions and time share one compact footer row instead of stacking into separate vertical levels. Albums retain their intentional cropped mosaic, while full-screen viewers always expose the complete authenticated source, move adjacent pages continuously under a horizontal drag, and dismiss with an upward single-finger swipe when unzoomed. Selection adds Violet boundary/checks without permanently changing normal geometry.

The composer is a calm Milk/Warm strip with a warm field, quiet attachment control, Violet send control, and existing voice gesture behavior. The sparkle cooperative launcher stays prominent in the header. Back, identity, and frequent actions form separate compact Telegram-like header segments. The chat identity header, pinned banner, and active voice-note controls remain outside the Android keyboard-resized timeline/composer region; a height-only IME guard may shrink that region but must never translate the header. Opening the keyboard preserves the visible history anchor unless the reader is already near the bottom. Pinned/reply/edit states use compact soft group surfaces.

### Profile

Milk canvas with one large Pink identity/portrait stage as the dominant expressive area. Profile photos form a swipeable, edge-to-edge hero gallery with a quiet position counter; tapping any photo opens the authenticated continuous full-screen carousel. The identity card overlaps the lower edge of the hero so name and biography remain anchored while photos stay visually primary. Owners manage the currently visible photo from the top-right ellipsis menu—add multiple photos, make primary, or delete—rather than through a separate management card or camera badge. Contacts return to neutral Warm rows. It is a personal identity page, not a social feed or collage of colored cards.
...
The launcher gives real games their own **Games / Игры** section before cooperative prompts and collections. A game board uses one calm neutral field plus Violet and Lime for selection/turn state; traditional piece colors remain recognizable but do not introduce decorative palette noise. The current turn, both identities, roles, round, and series score stay visible. Legal targets have a shape as well as color. Completion presents one dominant **Play again / Играть ещё** action; the first tap requests a rematch and the second participant accepts it in the same sheet. Never require reopening or scrolling through the launcher between rounds, duplicate the game title, or create a new chat card for each move.
