...
Draw & Guess uses an authorized ephemeral Socket.IO preview stream for throttled bounded vectors; guesses are accepted during that live preview, while the revisioned HTTP command remains the only durable final drawing. Color Hunt uses the ordinary media job table with the `color-collage` operation introduced by migration `0019_activity_collages.sql`. A participant may add the remaining photos in one bounded command. Nine ready source attachments produce one immutable, share-ready 2160 × 2160 PNG plus a lightweight WebP thumbnail in the media worker, and the activity scheduler advances the anchor revision and publishes the resulting attachment to both cached chats. The activity detail screen uses stale-while-revalidate rendering so each per-photo revision updates its tile without replacing the entire sheet with a loader.
