...
import { conversations, conversationMembers } from "../db/schema.js";
import { eq, and, ne } from "drizzle-orm";
import { nanoid } from "nanoid";
/**
 * Finds or creates a 1-on-1 DM conversation between two users
 */
export async function getOrCreateDM(userAId, userBId) {
    if (userAId === userBId) {
    ...
    if (aConvIds.length > 0) {
        ...
        const dmConv = commonMemberships.find((m) => aConvIds.includes(m.conversationId) && m.conversation?.type === "dm");
        if (dmConv) {
...
export async function getUserConversations(userId) {
    ...
    for (const m of memberships) {
        ...
        const otherMember = await db.query.conversationMembers.findFirst({
            where: and(eq(conversationMembers.conversationId, conv.id), ne(conversationMembers.userId, userId)),
            with: {
                user: {
                    columns: {
                        id: true,
 ...
 */
export async function checkUserAccessToConversation(userId, conversationId) {
