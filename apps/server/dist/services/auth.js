...
export async function registerUser({ inviteCode, username, password, displayName, }) {
...
export async function createInviteCode(adminUserId, code) {
...
export async function getInviteCodes(adminUserId) {
    const user = await db.query.users.findFirst({
        where: eq(users.id, adminUserId),
    });
    if (!user || !user.isAdmin) {
